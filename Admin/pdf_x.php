...
if(isset($_SESSION["restuname"]) && isset($_SESSION["restprofile"]) && ($_SESSION["restprofile"]=="J" || $_SESSION["restprofile"]=="A" || $_SESSION["restprofile"]=="T")){
    ...
    if(count($rsServ)>0){
        ...
        $tab_resumen='<table border="1" cellpadding="2"><tr><td>FORMA DE PAGO</td><td>+BASE</td><td>+VENTAS</td>
        <td>+R.CARTERA</td>
        <td>+AJUSTES</td>
        <td>+PROPINAS</td><td>-GASTOS</td><td>=SUGERIDO</td><td>CONTEO</td><td>DIFERENCIA</td></tr>';
        $totfp=0;
        $tot_caja=0;
        $tot_otros=0;
        $tot_v=0;
        ...
        if(count($resultInt)>0){
            ...
            foreach($resultInt as $rwI){
                ...
                //if($valor>0){
                    ...
                    $tab_resumen.='<tr><td>'.$forma.'</td><td align="right">'.number_format($ba_caa,0).'</td>
                    <td align="right">'.number_format($tvent,0).'</td>
                    <td align="right">'.number_format($car,0).'</td>
                    <td align="right">'.number_format($tprop,0).'</td><td align="right">'.number_format($tgast,0).'</td><td align="right">'.number_format($valor,0).'</td><td align="right">'.number_format($valorcu,0).'</td><td align="right">'.number_format($descu,0).'</td></tr>';
        ...
        $tab_resumen.='<tr><td>TOTAL</td><td align="right"><b>'.number_format($base_caja,0).'</b></td><td align="right"><b>'.number_format($tot_v,0).'</b></td>
        <td align="right"><b>'.number_format($tot_car,0).'</b></td>
        <td align="right"><b>'.number_format($tot_p,0).'</b></td>
        <td align="right"><b>'.number_format($tot_g,0).'</b></td><td align="right"><b>'.number_format($tot_t,0).'</b></td><td align="right"><b>'.number_format($tot_c,0).'</b></td><td align="right"><b>'.number_format($tot_d,0).'</b></td></tr></table>';

        $resumen_lineal.='<b>TOTALES</b><br />Base:'.number_format($base_caja,0).'<br />Ventas:'.number_format($tot_v,0).'<br />Rec. cartera:'.number_format($tot_car,0).'<br />Propinas:'.number_format($tot_p,0).'<br />Gastos:'.number_format($tot_g,0).'<br />Sugerido:'.number_format($tot_t,0).'<br />Conteo: '.number_format($tot_c,0).'<br />Conteo: '.number_format($tot_d,0).'';
    ...
    $out_htm.='<tr><td>
    <hr />
    <h4>SERVICIO No. '.$idserv.' - FECHA: '.$fecha.'</h4><br />
