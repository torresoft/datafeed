...
function initDF(){
  ...
  datafeed.on("mesa_ocupada", function(info){
    ...
    if(tipo!="D"){
      ...
      var statusBadge = "<span class='badge' style='background:#f44336; color:#fff; padding:4px 8px; border-radius:10px; font-size:9px; font-weight:600;'><i class='fa fa-cutlery'></i></span>";
      var headerDiv = $("#infoboxa_"+id_mesa).find(".inner > div").first();
      headerDiv.find(".badge").remove();
  ...
  datafeed.on("mesa_liberada", function(info){
    ...
    $("#infoboxa_"+id_mesa).find(".badge").remove();
  ...
  datafeed.on("mesa_enviada", function(info){
    ...
    if(tipo!="D"){
      ...
      // Actualizar badge de estado
      var statusBadge = "<span class='badge' style='background:#ff9800; color:#fff; padding:4px 8px; border-radius:10px; font-size:9px; font-weight:600;'><i class='fa fa-fire'></i></span>";
      var headerDiv = $("#infoboxa_"+id_mesa).find(".inner > div").first();
      headerDiv.find(".badge").remove();
      headerDiv.append(statusBadge);
      tryhovers();
  ...
  datafeed.on("cambios", function(rows){
    ...
    $(".lasmesas").each(function(){
        ...
        if(arrayPeds[idm]==undefined){
          ...
          // Remover badge de estado
          $("#infoboxa_"+idm).find(".badge").remove();
          $(this).off();
          ...
          if(estado_p=="0000-00-00 00:00:00"){
            ...
            // Actualizar badge de estado
            var statusBadge = "<span class='badge' style='background:#f44336; color:#fff; padding:4px 8px; border-radius:10px; font-size:9px; font-weight:600;'><i class='fa fa-cutlery'></i></span>";
            var headerDiv = $("#infoboxa_"+idm).find(".inner > div").first();
            headerDiv.find(".badge").remove();
            headerDiv.append(statusBadge);
          }else{
