...
function initDF(){
  ...
  datafeed.on("mesa_ocupada", function(info){
    ...
    if(tipo!="D"){
      ...
      $("#tbl_"+id_mesa).find(".tdr").text(usr);
      $("#tbl_"+id_mesa).find(".info-box").removeClass("bg-grey bg-orange");
      $("#tbl_"+id_mesa).find(".info-box").addClass("bg-red");
      $("#tbl_"+id_mesa).find(".info-box-number").html("P#"+id_pedido+"<small class='pull-right'>$0</small>");
      $("#tbl_"+id_mesa).off();
  ...
  datafeed.on("mesa_liberada", function(info){
    let id_mesa=info.id_mesa; 
    console.log(id_mesa);
    $("#tbl_"+id_mesa).find(".info-box").removeClass("bg-red bg-orange");
    $("#tbl_"+id_mesa).find(".info-box").addClass("bg-grey");
    let tipo=$("#tbl_"+id_mesa).attr("t");
    $("#tbl_"+id_mesa).find(".tdr").text("");
    $("#tbl_"+id_mesa).find(".info-box-number").html("Libre");
    $("#tbl_"+id_mesa).find(".progress-description").text("");
    $("#tbl_"+id_mesa).attr("timer",0);
    $("#tbl_"+id_mesa).off();
    $("#tbl_"+id_mesa).removeAttr("lnk-tsf");
    $("#tbl_"+id_mesa).removeAttr("lnk-cont");
    $("#tbl_"+id_mesa).click(function(){
      getDialog('mviews.php?flag=create_pedido&id_mesa='+id_mesa+'&t='+tipo,'600','Crear Pedido');
    });
  ...
  datafeed.on("mesa_enviada", function(info){
    ...
    let tipo=$("#tbl_"+id_mesa).attr("t");
    if(tipo!="D"){
      $("#tbl_"+id_mesa).find(".info-box").removeClass("bg-red bg-grey");
      $("#tbl_"+id_mesa).find(".info-box").addClass("bg-orange");
      tryhovers();
    }else{
      $("#tbl_"+id_mesa).find(".info-box").removeClass("bg-red bg-orange");
      $("#tbl_"+id_mesa).find(".info-box").addClass("bg-grey");
  ...
  datafeed.on("cambios", function(rows){
    ...
    $(".lasmesas").each(function(){
        ...
        if(arrayPeds[idm]==undefined){
          $(this).find(".info-box").removeClass("bg-red bg-orange");
          $(this).find(".info-box").addClass("bg-grey");
          $(this).find(".tdr").text("");
          $(this).find(".info-box-number").html("Libre");
          $(this).find(".progress-description").text("");
          $(this).off();
          $(this).click(function(){
            getDialog('mviews.php?flag=create_pedido&id_mesa='+idm+'&t='+tipo,'600','Crear Pedido');
          });
          ...
          $(this).find(".progress-description").text("");
          $(this).find(".tdr").text(usr);
          if(estado_p=="0000-00-00 00:00:00"){
            $(this).find(".info-box").removeClass("bg-grey bg-orange");
            $(this).find(".info-box").addClass("bg-red");
            
          }else{
            $(this).find(".info-box").removeClass("bg-grey bg-red");
            $(this).find(".info-box").addClass("bg-orange");
          }
          $(this).find(".info-box-number").html("P#"+id_pedido+"<small class='pull-right'>$"+total_ped+"</small>");
  ...
  datafeed.on("act_mesa", function(rows){
    ...
    $("#tbl_"+id_mesa).find(".info-box-number > small").html("$"+number_format(total_ped,0));
  });
...
function calcTimers(){
  $(".lasmesas").each(function(){
    ...
    if(apertura!="0" && apertura!=undefined){
      ...
      $("#tbl_"+id_mesa).find(".progress-description").text(minutos+" "+units);
    }else{
      $("#tbl_"+id_mesa).find(".progress-description").text("");
    }
