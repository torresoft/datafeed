...
function initDF(){
  ...
  datafeed.on("mesa_ocupada", function(info){
    ...
    if(tipo!="D"){
      ...
      var nombre=$("#tbl_"+id_mesa).find("p strong").text();
      ...
      $("#tbl_"+id_mesa).find("h3").first().html(mesan);
  ...
  datafeed.on("cambios", function(rows){
    ...
    $(".lasmesas").each(function(){
        ...
        var nombre=$(this).find("p strong").text();
        var mesan=nombre.replace("MESA ","").replace("mesa ","");
  ...
  datafeed.on("act_mesa", function(rows){
    ...
    $("#tbl_"+id_mesa).find("span.pull-right").first().find("small").html("$"+number_format(total_ped,0));
  });
